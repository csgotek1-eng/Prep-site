...
| Header CTA refined: "Get a Quote" (desktop/tablet), "Get Quote" (phones below `sm`); 48/44/40 px height, flat (no shadow), `rounded-lg`, `duration-150` hover/active darken only, no scale (2026-09-25) | Owner, explicit brief | Still `VARIANTS.header` in CalculatorModal.tsx, still exactly two `<CalculatorTrigger variant="header">` call sites, destination/behaviour unchanged, `:focus-visible` untouched | The retired "Get a Quote" label (§ vocabulary audit) named two different destinations at once; reinstated here because nothing else on the site now uses the phrase, so there is no live collision — flagged, not silently reverted |
