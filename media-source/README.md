...
| `incoming-shipment-inspection.source.webp` | `public/media/batch-photos/incoming-shipment-inspection.webp` | **Added 2026-09-24** (owner request) for /batch-photos, placed after the first text block. Provenance not stated by the owner — treat as an owner-supplied illustration, not licensed and not Dockentra's own bench. Shows unpacked jars in a box on a steel table with a count sheet, one jar visibly cracked; no people. **No crop, no resize:** the full 1672×941 frame, metadata stripped, re-encoded WebP q86 (checked side by side at the cracked jar and the count sheet). 149 KB → 110 KB. |
