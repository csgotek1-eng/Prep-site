...
| `NEXT_PUBLIC_OWNER_CONTACT_EMAIL` | Optional. The owner's public contact address, rendered on the page by `src/lib/site-contact.ts`. When unset, `siteContact.email` is `null` and every "Email us" action falls back to the enquiry form — no address is invented. Public by design (it is printed for visitors), hence the prefix. |
| `REVIEWS_PERSISTENCE` / `REVIEWS_STORE_FILE` | Customer-review storage. Usually unset (follows `PRICING_PERSISTENCE`); `file` for development, `supabase` in production. Supabase mode needs migration `0008_website_reviews.sql` applied. Fail closed: unconfigured means the submission endpoint answers 503 rather than accepting a review it cannot store. |
| `PRICING_NOTIFICATION_TO` | Where the "somebody asked for a price" email goes. Defaults to the published contact address. A free-mail address is fine here — it is a recipient, not a sender. Needs `PRICING_EMAIL_DELIVERY_MODE=resend` to send at all. |
| `NEXT_PUBLIC_GOOGLE_ANALYTICS_ID` | GA4 Measurement ID (`G-XXXXXXXXXX`), from Google Analytics → Admin → Data streams → your web stream. Unset = no Google script loads and the CSP names no Google host. Read at **build** time. The tag loads with Consent Mode v2 defaulting all storage to denied (cookieless) because the site has no consent banner yet. |
| `PRICING_STORE_FILE` | Optional path for the development pricing store JSON file (default `./data/pricing-store.json`, gitignored). |
| `SUPABASE_PUBLIC_URL` / `SUPABASE_PUBLISHABLE_KEY` | Supabase project URL and publishable key (needed only for the supabase modes). Read server-side and passed to the admin login as props — deliberately not `NEXT_PUBLIC_`. `SUPABASE_PUBLIC_URL` is additionally read at **build** time to pin the CSP's `connect-src` to that exact origin (otherwise it falls back to a `*.supabase.co` wildcard). |
...
See [docs/BRAND_ASSETS.md](docs/BRAND_ASSETS.md). The favicon and Apple touch icon are `src/app/icon.png` and `src/app/apple-icon.png` (crops/resamples of the approved mark), and the 1200×630 Open Graph image is generated by `src/app/opengraph-image.tsx`; `public/brand/` holds the approved logo files.
...
Notes: requires Node.js 20.9+ (declared in `package.json` engines; Vercel default is fine); pages are static except the API routes, which run as serverless functions. No `vercel.json` is needed — zero-config Next.js support covers everything, including the security headers set in `next.config.ts`: CSP (with `connect-src` pinned to the configured Supabase origin), HSTS, `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy` and `Permissions-Policy`. Rate limiting on the lead endpoints is durable and shared across serverless instances via Supabase (`src/lib/rate-limit.ts`, migration 0004), with an in-memory burst layer per instance. Any platform that runs `npm run build` + `npm run start` also works. `GET /api/health` reports configuration readiness (`{ok, pricing, leadStore}`) without touching the database.
